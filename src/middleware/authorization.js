      ...
      if (user) {
        bcrypt.compare(
          ...
          function (err, result) {
            if (err) {
              res.json({
                success: responses.INTERNAL_SERVER_ERROR.CODE,
                message: "Server dose not respond",
              });
            }

            if (!result) {
              res.json({
                success: responses.UNAUTHORIZED_REQUEST.CODE,
        ...
        if (authorization.split(" ")[0] == "Bearer") {
          ...
          jwt.verify(token, process.env.JWT_SECRET, async (error, payload) => {
            try {
              ...
              if(!user) {
                throw new Error("Authorization token not expired nor user was removed in the system.");
              }
              req.user = user;
              next();
            } catch (error) {
